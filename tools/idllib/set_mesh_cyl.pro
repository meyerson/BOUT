...
; simple function to 
  ...
  output = interpol(smooth(temp,N_ELEMENTS(x)/9.0,/edge_truncate),x,r)
  return, output
end
...
pro set_mesh_cyl, d1, d2, d3, plotmesh=plotmesh, export=export, noreset=noreset,   $
                  ...
                  ra=ra, rb=rb,  rc = rc,$ ; common parameters for all profiles
...
; Radial profiles, parameters common for all profiles
 ...
 if not keyword_set(rc) then rc=(rb+ra)/2.0 
   ...
   IF keyword_set(EXPORT) then begin
    if not keyword_set(PATH) then PATH='.' 
      ...
      file3=PATH + '/uedgeout.pdb'
        ...
        help,d1,d2,d3
        save,d1,filename= file1
        save,d2,filename= file2
        save,d3,filename= file3
