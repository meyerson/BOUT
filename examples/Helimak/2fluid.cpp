...
int physics_init()
{
  ...
  /**************** CALCULATE METRICS ******************/
  //output.write("Metric: %e, %e, %e",g11.Mean(),g22.Mean(),g33.Mean());
...
int physics_run(real t)
{
 ...
 jpar = ((Nit*(1.71)*Grad_par(Tet,CELL_YLOW)+Tet*Grad_par(Nit, CELL_YLOW)) - (Nit*Grad_par(phi, CELL_YLOW)))/(fmei*0.51*nu); 
  ...
  if(evolve_vi) {
    ...
    //F_Vi -= Grad_par(pei0)/Nit - Grad_par(pei0)/Ni0;


    //Dmitry's addition based on Zeiler and Drake
    F_Vi -= pi * V_dot_Grad(b0xcv, Vi0)+ pi0 * V_dot_Grad(b0xcv, Vi);
  ...
  if(evolve_te) {
    ...
    //Dmitry's addition based on Zeiler and Drake, add nonlinear terms later,
    // again these are terms that are present in Xu's paper
    
    F_Te += (.71)*(0.6666667/Ni0)*Te0*Div_par(jpar);
    
    F_Te -= (0.6666667)* Te0*Div_par(Vi);
  ...
  if(evolve_ti) {
    ...
    // Dmitry' additions, these terms are in the BOUT documentation, maybe they
    // were cut from BOUT++? 

    F_Ti -= Ti0*(0.6666667) * Div_par(Vi);
    F_Ti += Ti0*(0.6666667/Ni0) * Div_par(jpar);
