...
BoutReal*** Field3D::getData() const //return pointer to 2d array
{
#ifdef CHECK
  if(block ==  NULL)
...
const Field3D yfilter(const Field3D &var, int N0)
{
  ...
  for(int jx=0;jx<mesh->ngx;jx++)
    for(int jz=0;jz<mesh->ngz-1;jz++) {
      //var.getZArray(jx, 4, v);
      //output.write("%d \n ",v.size());
      for(jy=0;jy<mesh->ngy;jy++)
	d[0][jy] = var.block->data[jx][jy][jz];
      
      //x and z fixed at this point
      rfft(d[0], ncy, fy);

      for(jy=0;jy<=ncy/2;jy++){ 	
	if(jy != N0) {
	  fy[jy] = 0.0;
	}
      }
...
const Field3D yfilter(const Field3D &var, int N0)
{
  ...
  //static dcomplex *fy = (dcomplex*) NULL;
  
  Field3D temp2;
  //Field3D f; //demands a nx X ny X nz geometry not what we want here . .
  static dcomplex **f = NULL, *fy;
  int jx, jy, jz;
  ...
  if(f == NULL) {
    //.allocate memory
    f = cmatrix(mesh->ngz,mesh->ngy);
    temp = rmatrix(mesh->ngz,mesh->ngy);
    output.write("mesh size: %d,%d \n",mesh->ngz,mesh->ngy);
    //output.write("var size: %d, %d \n",
    //f = new dcomplex[mesh->ngx][mesh->ngz][mesh->ngy]
    fy = new dcomplex[ncy/2 + 1];
    //temp2.block->data[][]
    //BoutReal** temp = new BoutReal[mesh->ngz];
    
  }

 

  //allocate the new field by hand
  
  memblock3d* nb;
  memblock3d* free_block = NULL;
  memblock3d* blocklist=NULL;
  int nblocks = 0;
  #pragma omp critical
  {
    if(free_block != NULL) {
      ...
      nb->data = r3tensor(mesh->ngx, mesh->ngz, mesh->ngy);
      nb->refs = 1;
      nb->next = NULL;
  ...
  #pragma omp parallel for
  for(jx=0;jx<mesh->ngx;jx++) {
    //result.block->data[jx] = var.block->data[jx];
    for (jz=0;jz<mesh->ngz;jz++) {
      for(jy=0;jy<mesh->ngy;jy++) {
	nb->data[jx][jz][jy] = var.block->data[jx][jy][jz];
	result.block->data[jx][jy][jz] = var.block->data[jx][jy][jz];
	//result.block->data[jx][jy][jz] = 0.0;
      }
    }//end rearrange  
    for(jz=0;jz<mesh->ngz;jz++) {
      
      rfft(nb->data[jx][jz], ncy, fy); // Forward FFT
	...
	if(jy != N0) {
      ...
      irfft(fy, ncy, nb->data[jx][jz]); // Reverse FFT

      nb->data[jx][jz][ncy] = nb->data[jx][jz][0];
    
    } // end filter
    
    for(jy=0;jy<mesh->ngy;jy++) {
      for (jz=0;jz<mesh->ngz;jz++) {
	result.block->data[jx][jy][jz] =nb->data[jx][jz][jy];
      }
    } //end de-rearrange
...
#ifdef TRACK
  result.name = "yfilter("+var.name+")";
#endif
  //result.block = var.block;
  result.location = var.location;
